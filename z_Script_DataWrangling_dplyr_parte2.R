...
# Dados -------------------------------------------------------------------
dataset_wdi <- read_excel("raw_datas_exemplos/(2.2) WDI World Bank.xlsx")
...
# option ( ): alterar limites de print no console -------------------------
...
# Simplificar planilha ----------------------------------------------------

# rename ( ) --------------------------------------------------------------
# Para facilitar, vamos iniciar simplificando os nomes das colunas
# Alteração do nome por meio da posição da coluna
...
# Selecionando por posição de linha: da linha 1 até a 382572. Então elimina
# as outras linhas
...
dataset_wdi_saude <- dataset_wdi %>% 
  filter(str_detect(topico, "^Health")) # começam com Health
...
# id_cols: quais colunas queremos como identificador das observações
# names_from: o que queremos nas variáveis (nas colunas). Todo mundo que 
# está na coluna "serie" vai virar uma coluna do novo dataset
# values_from: onde estão os valores das variáveis
...
# purrr::discard ( ) ------------------------------------------------------
# descartar todas as variáveis que possuem todas as observações como NA
